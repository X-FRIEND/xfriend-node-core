...
const { graylog } = require('../../../../v1/src/logger/formatters');
...
describe('graylog formatter', () => {
  ...
  it('return object with log_level, log_message and log_timestamp undefined when event is not defined', () => {
    // given
    const expectedResult = { ...emptyOutput };
    // when
  ...
  it('transform level into log_level', () => {
    // given
    const level = 'debug';
    const expectedResult = { ...emptyOutput, log_level: level };
    const event = { level };
    // when
  ...
  it('transform message into log_message', () => {
    ...
    const expectedResult = { ...emptyOutput, log_message: message };
    const event = { message };
    // when
    const formattedEvent = graylog(event);
  ...
  it('transform timestamp into log_message', () => {
    ...
    const expectedResult = { ...emptyOutput, log_timestamp: timestamp };
  ...
  it('expose fields from log message', () => {
    ...
    const expectedResult = { ...message, ...emptyOutput, log_message: message };
