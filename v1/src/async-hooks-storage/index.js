...
// Special log function for not triggering new asyncIds
// eslint-disable-next-line complexity
function syncLog(logStr, graphStr, asyncId, remove) {
  if (hasLog) fs.writeSync(1, `${logStr}\n`);
...
asyncHooksStorage.showStats = () => {
  ...
  fs.writeSync(
    ...
    `\nAsyncID      : ${hooksLast}\nGraph entries: ${graphEntries}\t(${
      graphEntries - lastGraphEntries
    })\nHooks count  : ${
      hooksCount
    }\t(${
      hooksCount - lastHooksCount
    })\nHooks size   : ${
      storageSizeKB
    } KB\t(${
      storageSizeKB - lastStorageSizeKB
    })\nHeap total   : ${
      Number(heapTotal / 1000000)
    } MB\nHeap used    : ${
      Number(heapUsed / 1000000)
    } MB\n`,
  );
