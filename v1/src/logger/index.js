...
class Logger {
  ...
  setSessionId(sessionId) {
    this.contextData.context = { ...this.contextData.context, sessionId };
  }

  /** @private */
  output(message, additionalArguments, outputType = loggerLevels.log) {
    if (this.shouldSuppressOutput(message, outputType)) return;
    const event = this.serializer.serialize(message, additionalArguments, outputType, this.contextData);
